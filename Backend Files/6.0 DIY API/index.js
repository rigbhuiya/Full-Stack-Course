...
app.get("/random",(req,res)=>{
  const id=Math.floor(jokes.length*Math.random());
  res.json(jokes[id]);
})
...
app.post("/jokes",(req,res)=>{
  ...
  jokes.push(newJoke);
  console.log(jokes.slice(-1)[0].jokeText);
  res.json(newJoke);
})
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const index=jokes.findIndex((joke)=>joke.id===rid);
...
app.patch("/jokes/:id",(req,res)=>{
  const rid=parseInt(req.params.id);
  const existingJoke=jokes[rid-1];
  const updateJoke={
    id:rid,
  ...
  console.log(jokes[rid-1]);
  res.json(jokes[rid-1]);
})
